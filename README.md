...
1. **Syncs service orders from an ERP export** (`src/baan_sync.py`) — replaces manually typing a "500 number" into a spreadsheet. Once an order exists in the ERP, it exists in the system automatically.
...
│   ├── baan_sync.py               # ERP sync -> service_orders
...
├── data/
│   └── baan_export.csv            # simulated ERP export (sync source)
├── docs/
...
See `docs/workflow_comparison.md` for the full list — briefly: connect `baan_sync.py` to an actual ERP feed instead of a CSV, add authentication to the Streamlit app, auto-send quote emails instead of a manual "Mark as Sent" click, and run `follow_up.py` on a schedule with Slack/email alerts instead of requiring someone to open the dashboard.
