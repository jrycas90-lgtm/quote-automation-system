...
- **The ERP system**: the actual system of record for service orders, but disconnected from pricing entirely. A human had to manually copy the service order number from the ERP into the spreadsheet.
...
| Manual data entry | The service order number was retyped by hand instead of pulled from the ERP directly |
...
| Manual "500 number" entry | `erp_sync.py` syncs service orders from an ERP export automatically — the number exists in the system the moment it exists in the ERP |
...
- Replace `data/erp_export.csv` with an actual scheduled pull from the ERP (ODBC, ERP export job, or REST API depending on the ERP version)
